...
public abstract class Graph {
    ...
    public List<Node> getNeighbors(Node node) {
        ...
        for (Node neighbor : node.getNeighbors())
            if (canMove(node, neighbor)) neighbors.add(neighbor);
        return neighbors;
    }
    ...
    public static class MCGraph extends Graph {
        ...
        public Location getLocation(Node.Node3D node) {
            return new Location(world, node.x + 0.5, node.y, node.z + 0.5);
        }

        public boolean isInBound(Node.Node3D node) {
        ...
        public boolean canMove(Node from, Node to) {
            if (!(from instanceof Node.Node3D && to instanceof Node.Node3D)) return false;
            return canMove(getBlock((Node.Node3D) from), getBlock((Node.Node3D) to));
        }
        ...
        public List<Node> getNeighbors(Node node) {
            ...
            if (!(node instanceof Node.Node3D)) return neighbors;

            Block from = getBlock((Node.Node3D) node);
            ...
            for (Node neighbor : new ArrayList<Node>(neighbors)) {
                Block block = getBlock((Node.Node3D) neighbor);
                ...
                if (!canMove(from, block)) {
                    ...
                    for (int y = (int) -Math.ceil(maxFall); y <= maxJump; y++) {
                        ...
                        if (canMove(from, other)) neighbors.add(getNode(other));
                    }
                }
            }
